...
```json
// 단일 사이클 레코드 예시
// 해석: 1층→잭팟(23스핀), 2층→층천장(40스핀), 3층→잭팟(8스핀), 4층→글로벌천장 발동(5층 직행)
{
  "season_number": 2,
  "select_count": 45,
  ...
  "floor_spin_counts":         [23, 40, 8, 0, 0]
}
```

`global_pity_triggered = 1`이면 4층에서 글로벌 천장이 발동됐음을 의미한다.  
`floor_spin_counts[3] == 0`은 4층이 글로벌 천장으로 스킵됐음을 함께 나타낸다.  
...
| 글로벌 천장으로 스킵된 중간 층 | 의도적 미수집 | 실제 플레이하지 않은 층이므로 제외, `floor_spin_counts = 0`으로 표시됨 |
