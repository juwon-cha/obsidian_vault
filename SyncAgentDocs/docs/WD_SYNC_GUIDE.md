...
FROM과 TO의 동일 이름 메서드가 **타입 파라미터(특히 enum)는 다를 수 있다.** 그리고 한 메서드명에 4-5개의 오버라이드 시그니처가 공존하면 **그 중 일부만 변환해서는 안 된다.** 전수 변환이 원칙.

**자주 마주치는 축 변경:**

| FROM 시그니처 | TO 시그니처 | 처리 |
|--------------|-----------|------|
| `TakeDamage(float damage, EUnitRaceType raceType, ...)` | `TakeDamage(float damage, EElementType elementType, ...)` | enum 축 변경 — 호출자·override 모두 EElementType으로 |
| `Heal(float amount, EHealSourceType src)` | `Heal(float amount, EBuffType src)` | enum 축 변경 — 호출자에서 매핑 테이블 필요 시 명시 |
| `Spawn(EnemyData data, EUnitRaceType race)` | `Spawn(EnemyData data, EElementType element)` | enum 축 변경 |

**다중 오버라이드 함정:**

베이스 클래스에 동명의 시그니처가 N개 있고 파생 클래스에서 일부만 override하면, **나머지 시그니처로 호출이 들어올 때 base가 실행되어 의도된 처리가 누락된다.** sync 시 반드시 베이스의 시그니처 전수를 추출하고 파생 클래스의 override 카운트를 일치시킨다.

```bash
# 베이스의 동명 메서드 시그니처 전수 추출
grep -nE "(public|protected).*(virtual|abstract).*TakeDamage\(" \
  "{to}/Assets/_Project/1_Scripts/Core/Units/UnitBase.cs"

# 파생 클래스의 override 카운트
grep -cE "override.*TakeDamage\(" \
  "{to}/Assets/_Project/1_Scripts/.../{DerivedUnit}.cs"

# 두 카운트가 일치하지 않으면 → 누락된 시그니처로 들어오는 데미지가 base로 빠짐
```

**축 변경 매핑 테이블 작성 의무**: enum 축이 바뀌는 경우 분석 문서 섹션 2에 다음 매핑을 명시한다:

```
## TakeDamage 시그니처 축 변경 매핑
FROM EUnitRaceType → TO EElementType

| FROM enum 값 | TO enum 값 | 비고 |
|-------------|-----------|------|
...
> **참조**: Phase 6 Step 0-7에서 이 규칙 위반을 자동 검사한다.
...
> **⚠️ 사전 조건**: FROM 코드를 읽으려면 FROM 프로젝트의 로컬 경로(`{from_local}`)가 확정되어 있어야 한다. `/sync` 에이전트를 사용하는 경우 Phase 0에서 자동으로 설정된다. 수동으로 진행하는 경우 섹션 8을 참조한다.
...
```bash
# {from_local} = Phase 0에서 확정된 FROM 프로젝트 로컬 경로
# 키워드는 많을수록 좋다 — 클래스명·타입명·인터페이스명·SaveData명·시스템 접두어 전부 포함
grep -rl "FooManager\|FooTypes\|IFooCapable\|FooSaveData\|EFooSlot\|Foo" "{from_local}/Assets" --include="*.cs"
```
...
| **ADAPTED** | FROM 전용 패턴(규칙 1~16)만 변환하면 됨, 의존성 전부 TO에 존재 | 규칙 적용 후 복사 |
...
grep -A 30 "class FooSaveData" "{from_local}/Assets/_Project/1_Scripts/.../FooSaveData.cs"
...
grep -n "ServiceAccessor\|Inject\|MessageBroker\|using Geuneda\|using UniRx" "{from_local}/Assets/_Project/1_Scripts/UI/SomeUI.cs"
```
...
grep -n "\.[A-Z][a-zA-Z]*(" "{from_local}/{sync파일경로}" | grep -v "\/\/"
...
- 변환이 불가피한 부분(규칙 1~16)은 동일 줄 위치에서 인라인으로 변경
...
| 시그니처 축 변경 (규칙 16) | enum 파라미터 1개당 줄 변경 + override 누락 시그니처 추가 줄 수 |
...
섹션 7에 기재할 항목:
- 신규 생성 파일 전체의 `cp "{from_local}/{경로}" /tmp/from_{파일명}` + `diff` 명령어
...
| 시그니처 축 변경 미적용 | `cannot convert from 'EUnitRaceType' to 'EElementType'` | 호출자·override 모두 새 enum 축으로 변환 (규칙 16) |
| 다중 오버라이드 중 일부만 변환 | 런타임에 일부 데미지/효과가 base로 빠져 의도된 처리 누락 | 베이스 시그니처 전수 추출 후 모든 시그니처에 override (규칙 16) |
...
## 8. FROM 프로젝트 접근 — FROM_LOCAL 확정

`/sync` 에이전트는 Phase 0에서 FROM 소스에 따라 자동으로 로컬 경로(`from_local`)를 확정한다.

| FROM 소스 타입 | Phase 0 처리 | from_local 값 |
|--------------|------------|--------------|
| 로컬 경로 | 경로 존재 확인 후 그대로 사용 | 입력한 경로 그대로 |
| remote 브랜치 (`origin/temp-bunker/dev`) | `git worktree add` 로 임시 워크트리 생성 | `{TO_PATH}/.claude/worktrees/{id}/` |
| remote URL | `git clone --depth=1` 후 임시 경로 | 클론된 경로 |

`from_local`이 확정되면 FROM 파일은 일반 파일 시스템 명령으로 읽는다:

```bash
# FROM 파일 직접 읽기
cat "{from_local}/Assets/_Project/1_Scripts/Core/Managers/FooManager.cs"

# FROM에서 특정 클래스 참조 파일 전수조사
grep -rl "FooManager\|FooTypes" "{from_local}/Assets" --include="*.cs"

# FROM 파일 목록 전체 확인
find "{from_local}/Assets" -name "*.cs" | grep -i foo
```

### 수동으로 진행하는 경우

`/sync` 에이전트 없이 수동으로 진행할 때는 FROM 경로를 직접 지정한다:

```bash
# 로컬 경로인 경우 그대로 사용
FROM_LOCAL="/Volumes/repo/BunkerDefense"

# remote 브랜치인 경우 워크트리 생성
git remote add temp-bunker {FROM_repo_url}  # 최초 1회
git fetch temp-bunker dev
git worktree add .claude/worktrees/from-sync temp-bunker/dev
FROM_LOCAL=".claude/worktrees/from-sync"
```
