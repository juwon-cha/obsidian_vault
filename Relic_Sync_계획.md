...
**필수 패턴 (DamageCalculationManager.ChipEffect.cs와 동일하게):**
```csharp
...
public partial class DamageCalculationManager
{
    ...
    private RelicManager RelicManagerCached
    {
        get
        {
    ...
    public float GetRelicUnitDamageBonus(EUnitType unitType)
    {
        var relic = RelicManagerCached;
        if (relic == null) return 0f;  // 초기화 전 안전하게 0 반환
        return relic.GetUnitDamageBonus(unitType);
    }
}
```
...
> `EAcquisitionRouteType`도 WiggleDefender에 존재하면 `Relicgacha` 추가
...
| 파일 | 수정 내용 |
|------|-----------|
| `Core/Managers/Managers.cs` | ⚠️ **ManagerFactory.cs 아님** — `MANAGER_DEFINITIONS`에 `RelicManager` 추가 (priority: 334) |
...
| `Core/Managers/ContentUnlockManager.cs` | `Feature_Relic` 해금 조건 추가 |
| `Core/Managers/RedDotManager.cs` | 유물 레드닷 노드 (`Lobby.Relic.*`) 추가, `CheckRelicPityAvailability()`, `CheckRelicGachaTicketAvailability()` 추가 |
| `Core/Managers/FirebaseRemoteConfigManager.cs` | `IsRelicGachaEnabled` bool 프로퍼티 추가 |
| `Core/Managers/AcquisitionRouteManager.cs` | `Relicgacha` 경로 처리 추가 |
...
// 각 유닛 초기화 시 호출 (Start/Initialize 등):
...
### 기타 연동 검증
- [x] `DamageCalculationManager` → `RelicEffect.cs` partial 연동 (lazy-init 패턴)
- [x] `LobbyMainUI` → 유물 뽑기 버튼 + 해금/리모트컨피그 조건
- [x] `ContentUnlockManager` → `Feature_Relic` 처리
- [x] `RedDotManager` → `Lobby.Relic.*` 레드닷 노드
...
- [ ] ⚠️ `Feature_Relic` 해금 레벨/조건 — **기획 확인 필요 (BLOCKER)**
- [ ] ⚠️ Localization 키 등록 — **데이터시트 확인 필요 (BLOCKER)**
- [ ] ⚠️ 프리팹/스프라이트 에셋 sync (Phase 9) — **별도 작업 필요**
